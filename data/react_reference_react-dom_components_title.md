...
## 参考
### <title>
渲染 浏览器内置的 <title> 组件 以指定文档标题。可以在任何组件中渲染 <title>，React 会始终将相应的 DOM 元素放置在文档头部。
```
<title>我的博客</title>
```
参见下方更多示例。
...
有两个例外情况：
- 如果 <title> 在 <svg> 组件内部，则没有特殊行为，因为在这种情况下它不代表文档的标题，而是 SVG 图形的可访问性注释。
- 如果 <title> 具有 itemProp 属性，也没有特殊行为，因为在这种情况下它不代表文档的标题，而是页面特定部分的元数据。
### 陷阱
