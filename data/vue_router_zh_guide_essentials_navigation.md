...
```
jsconst username = 'eduardo/san martin'
...
```
构建字符串路径或带有 path 的对象时，请提供已编码的路径。使用 encodeURIComponent 编码每个动态片段。同样的规则也适用于 <RouterLink>：
```
vue<RouterLink :to="`/user/${encodeURIComponent(username)}`">User</RouterLink>
<RouterLink
...
```
jsrouter.push({ path: '/home', replace: true })
// 相当于
...
例子
```
js// 向前移动一条记录，与 router.forward() 相同
router.go(1)
