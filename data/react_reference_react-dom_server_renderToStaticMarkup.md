...
- 可选的 identifierPrefix：一个 React 使用 useId 生成的 ID 的字符串前缀，用于避免在同一页面上使用多个 root 时发生冲突。
#### 返回值
一个 HTML 字符串。
#### 注意事项
...
```
import { renderToStaticMarkup } from 'react-dom/server';// 路由处理程序语法取决于你的后端框架app.use('/', (request, response) => {  const html = renderToStaticMarkup(<Page />);  response.send(html);});
```
