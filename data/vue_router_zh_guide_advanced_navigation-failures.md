...
```
✨自信地编写 Vue 应用的 Vibe 代码RuleKitjsconst navigationResult = await router.push('/my-profile')
...
```
jsimport { NavigationFailureType, isNavigationFailure } from 'vue-router'
...
```
tsrouter.afterEach((to, from, failure) => {
  if (failure) {
