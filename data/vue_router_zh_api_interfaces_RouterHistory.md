...
delta |
number |
访问的距离。如果 delta < 0 则后退相应数量的记录，如果 > 0 则前进。 |
triggerListeners? |
boolean |
是否应该触发连接到该历史的监听器 |
#### 返回值 
...
Example
```
jsmyHistory.go(-1) // equivalent to window.history.back()
...
data? |
HistoryState |
可选的 HistoryState 以关联该导航记录 |
#### 返回值 
void
### replace 
▸ replace(to, data?): void
和 RouterHistory.push 相同，只是执行了 history.replaceState 以换掉 history.pushState。
#### 参数 
名称 |
类型 |
描述 |
