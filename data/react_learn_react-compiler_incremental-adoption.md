...
随着你信心的增加，添加更多目录：
```
// babel.config.jsmodule.exports = {  plugins: [    // 全局插件  ],  overrides: [    {      test: ['./src/modern/**/*.{js,jsx,ts,tsx}', './src/features/**/*.{js,jsx,ts,tsx}'],      plugins: [        'babel-plugin-react-compiler'      ]    },    {      test: './src/legacy/**/*.{js,jsx,ts,tsx}',      plugins: [        // 针对旧代码的不同插件      ]    }  ]};
```
### 使用配置编译器选项
你还可以使用重写来配置编译器选项：
```
// babel.config.jsmodule.exports = {  plugins: [],  overrides: [    {      test: './src/experimental/**/*.{js,jsx,ts,tsx}',      plugins: [        ['babel-plugin-react-compiler', {          // 选项 ...        }]      ]    },    {      test: './src/production/**/*.{js,jsx,ts,tsx}',      plugins: [        ['babel-plugin-react-compiler', {          // 选项  ...        }]      ]    }  ]};
```
## 使用 “use memo” 的选择加入模式
...
```
function TodoList({ todos }) {  "use memo"; // 这个组件在编译里的选项  const sortedTodos = todos.slice().sort();  return (    <ul>      {sortedTodos.map(todo => (        <TodoItem key={todo.id} todo={todo} />      ))}    </ul>  );}function useSortedData(data) {  "use memo"; // 这个组件在编译里的选项  return data.slice().sort();}
```
...
## 运行时控制
gating选项使你能够在运行时使用功能标志控制编译。这对于运行 A/B 测试或根据用户细分逐步推出编译器非常有用。
### 运行时控制工作原理
...
创建一个模块来导出你的门控函数：
```
// ReactCompilerFeatureFlags.jsexport function isCompilerEnabled() {  // 使用你的特性开关系统  return getFeatureFlag('react-compiler-enabled');}
```
## 故障排除
如果在采用过程中遇到问题：
- 使用 "use no memo" 临时排除有问题的组件
- 查阅 调试指南 以了解常见问题
- 修复 ESLint 插件识别出的 React 规则违规
- 考虑使用 compilationMode: 'annotation' 以更渐进的方式采用
## 下一步
- 阅读 配置指南 了解更多信息和选项
- 学习 调试技巧
- 查看 API 参考 获取所有编译器选项