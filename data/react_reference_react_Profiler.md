...
```
function onRender(id, phase, actualDuration, baseDuration, startTime, commitTime) {  // 对渲染时间进行汇总或记录...}
```
#### 参数
...
即使在分析构建中，也会在 React Performance 跟踪的 Component tracks 中标记 <Profiler> 封装的组件。
