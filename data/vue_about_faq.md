...
自 2016 年以来，Vue 的发展主要是通过赞助来保障的，我们在财务上是可持续的。如果你或你的企业从 Vue 中受益，请考虑赞助我们，以支持 Vue 的发展！
...
我们相信这为大多数的生态系统提供了足够长的时间来迁移到 Vue 3。然而，我们也理解可能会有无法在此时间轴上升级的团队或项目仍需满足其安全及合规需求。我们正在与业内专家合作为有这种需求的团队提供 Vue 2 的扩展支持——如果您的团队预期在 2023 年底之后仍然需要使用 Vue 2，请确保提前计划，详见 Vue 2 延长 LTS。
## Vue 使用什么开源协议？ 
Vue 是完全免费的开源项目，且基于 MIT License 发布。
## Vue 支持哪些浏览器？ 
最新版本的 Vue (3.x) 只支持原生支持 ES2016 的浏览器。这并不包括 IE11。Vue 3.x 使用的 ES2016 功能无法在旧版本的浏览器中进行兼容，如果你需要支持旧版本的浏览器，请使用 Vue 2.x 替代。
## Vue 可靠吗？ 
...
## Vue 体积小吗？ 
当你通过构建工具使用时，Vue 的许多 API 都是可以“tree-shake”的。例如，如果你不使用内置的  组件，它就不会被包含在最终的生产环境包里。
...
如果你是 Vue 的新手，我们在这里提供了一个两者之间宏观的比较。
如果你过去使用过选项式 API 且正在评估组合式 API，可以查阅组合式 API 常见问题。
