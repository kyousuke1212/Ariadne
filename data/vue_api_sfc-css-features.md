# 单文件组件 CSS 功能 
## 组件作用域 CSS 
当  标签带有 scoped attribute 的时候，它的 CSS 只会影响当前组件的元素，和 Shadow DOM 中的样式封装类似。使用时有一些注意事项，不过好处是不需要任何的 polyfill。它的实现方式是通过 PostCSS 将以下内容：
vue
...
### 与组合式 API 一同使用 
可以通过 useCssModule API 在 setup() 和  中访问注入的 class。对于使用了自定义注入名称的  块，useCssModule 接收一个匹配的 module attribute 值作为第一个参数：
js
