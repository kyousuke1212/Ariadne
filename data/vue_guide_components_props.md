...
在没有使用  的组件中，props 可以使用 props 选项来声明：
js
export default {
props: ['foo'],
setup(props) {
...
### 合并绑定时的合并行为 
当 v-bind 与同一组件上的显式绑定一起使用时，Vue 内部会调用 mergeProps() 来合并它们。合并策略取决于键的类型：
* 常规 props —— 以最后一个值为准：
vue-html



* 事件监听器 —— 当在 v-bind 对象中传入监听器时，使用 onEventName 键的约定。同一事件的所有处理函数都会被调用 (参见 v-on 监听器继承)：
vue-html

 console.log(2) }" />

* class 和 style 遵循类似的合并策略 (参见 class 与 style 的合并)。
完整的合并规则参见 mergeProps() API 参考。
## 单向数据流 
...
要声明对 props 的校验，你可以向 defineProps() 宏props 选项提供一个带有 props 校验选项的对象，例如：
js
defineProps({
// 基础类型检查
...
如果使用了基于类型的 prop 声明 ，Vue 会尽最大努力在运行时按照 prop 的类型标注进行编译。举例来说，defineProps 会被编译为 { msg: { type: String, required: true }}。
