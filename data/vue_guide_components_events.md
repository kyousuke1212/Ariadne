...
## 事件参数 
有时候我们会需要在触发事件时附带一个特定的值。举例来说，我们想要  组件来管理文本会缩放得多大。在这个场景下，我们可以给 $emit 提供一个额外的参数：
vue-html
...
## 声明触发的事件 
组件可以显式地通过 defineEmits() 宏emits 选项来声明它要触发的事件：
vue

defineEmits(['inFocus', 'submit'])


我们在  中使用的 $emit 方法不能在组件的  部分中使用，但 defineEmits() 会返回一个相同作用的函数供我们使用：
vue
...
defineEmits() 宏不能在子函数中使用。如上所示，它必须直接放置在  的顶级作用域下。
如果你显式地使用了 setup 函数而不是 ，则事件需要通过 emits 选项来定义，emit 函数也被暴露在 setup() 的上下文对象上：
js
export default {
...
要为事件添加校验，那么事件可以被赋值为一个函数，接受的参数就是抛出事件时传入 this.$emitemit 的内容，返回一个布尔值来表明事件是否合法。
vue
