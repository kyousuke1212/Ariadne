...
- 如果你将函数作为 nextState 传递，它将被视为 更新函数。它必须是纯函数，只接受待定的 state 作为其唯一参数，并应返回下一个状态。React 将把你的更新函数放入队列中并重新渲染组件。在下一次渲染期间，React 将通过把队列中所有更新函数应用于先前的状态来计算下一个状态。请参见下面的示例。
#### 返回值
set 函数没有返回值。
#### 注意事项
...
#### 基本的 useState 示例
1. 计数器（数字） 2. 文本字段（字符串） 3. 复选框（布尔值） 4. 表单（两个变量）
#### 第 1 个示例 共 4 个示例: 计数器（数字）
...
1. 传递更新函数 2. 直接传递下一个状态
#### 第 1 个示例 共 2 个示例: 传递更新函数
...
#### 状态中的对象和数组的示例
1. 表单（对象） 2. 表单（嵌套对象） 3. 列表（数组） 4. 用 Immer 编写简洁的更新逻辑
#### 第 1 个示例 共 4 个示例: 表单（对象）
...
1. 传递初始化函数 2. 直接传递初始状态
#### 第 1 个示例 共 2 个示例: 传递初始化函数
...
```
function handleClick() {  console.log(count);  // 0  setCount(count + 1); // 请求使用 1 重新渲染  console.log(count);  // 仍然是 0!  setTimeout(() => {    console.log(count); // 还是 0!  }, 5000);}
```
...
```
// 🚩 错误：在渲染过程中调用事件处理函数return <button onClick={handleClick()}>Click me</button>// ✅ 正确：将事件处理函数传递下去return <button onClick={handleClick}>Click me</button>// ✅ 正确：传递一个内联函数return <button onClick={(e) => handleClick(e)}>Click me</button>
```
