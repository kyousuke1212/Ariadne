...
```
在 React 编译器游乐场中查看此示例
React 编译器会自动应用等效的优化，确保你的应用只在必要时重新渲染。
深入探讨
#### React 编译器添加了哪种类型的记忆化？
显示更多
...
#### 优化重新渲染
React 允许你将 UI 表达为其当前状态的函数（更具体地说：其 props、state 和 context）。在其当前的实现中，当一个组件的状态发生变化时，React 会重新渲染该组件 及其所有子组件，除非你使用了 useMemo()、useCallback() 或 React.memo() 进行手动记忆优化。例如，在以下示例中，每当 <FriendList> 的状态变化时，<MessageButton> 都会重新渲染：
```
...
```
// **不会** 被 React 编译器记忆化，因为它不是一个组件或 hookfunction expensivelyProcessAReallyLargeArrayOfObjects() { /* ... */ }// 会被 React 编译器记忆化，因为它是一个组件function TableContainer({ items }) {  // 这个函数调用将被记忆化：  const data = expensivelyProcessAReallyLargeArrayOfObjects(items);  // ...}
```
在 React 编译器游乐场中查看此示例
...
因此，如果 expensivelyProcessAReallyLargeArrayOfObjects 在许多不同的组件中使用，即使传递了完全相同的 items，这个昂贵的计算也会被重复运行。我们建议在使代码变得更复杂之前，先 profiling 以确定计算是否真的那么昂贵。
## 我应该尝试这个编译器吗？
我们鼓励大家开始使用 React 编译器。虽然目前编译器仍是 React 的可选附加功能，但未来某些功能可能需要编译器才能完整运行。
### 使用起来安全吗？
React 编译器目前已经稳定，并且在生产环境中进行了广泛测试。虽然像 Meta 这样的公司已经在生产中使用了它，但是否将编译器部署到你的应用程序生产环境，将取决于你的代码库状况以及你对 React 规则 的遵循程度。
## 支持哪些构建工具？
React 编译器可以在多个构建工具中安装，例如 Babel、Vite、Metro 和 Rsbuild。
React 编译器主要是围绕核心编译器构建的一个轻量级 Babel 插件封装，其设计初衷是为了与 Babel 本身解耦。尽管编译器的第一个稳定版本主要仍然是一个 Babel 插件，但我们正在与 swc 和 oxc 团队合作，为 React 编译器构建一流的支持，这样你将来无需再将 Babel 添加到你的构建流程中。
Next.js 用户可以通过使用 v15.3.1 或更高版本来启用由 swc 调用的 React 编译器。
...
- 安装 - 安装 React 编译器并为你的构建工具进行配置
- React 版本兼容性 - 支持 React 17、18 和 19
- 配置 - 根据你的特定需求自定义编译器
- 渐进式采用 - 在现有代码库中逐步推广编译器的策略
- 调试与故障排除 - 识别并解决使用编译器时的问题
- 编译库 - 发布已编译代码的最佳实践
- API 参考 - 所有配置选项的详细文档
## 其他资源
除了这些文档外，我们还建议查看 React 编译器工作组 以获取有关该编译器的更多信息和讨论。