# \ 
 是在单文件组件 (SFC) 中使用组合式 API 的编译时语法糖。当同时使用单文件组件与组合式 API 时该语法是默认推荐。相比于普通的  语法，它具有更多优势：
* 更少的样板内容，更简洁的代码。
...
## 使用自定义指令 
全局注册的自定义指令将正常工作。本地的自定义指令在  中不需要显式注册，但它们必须遵循 vNameOfDirective 这样的命名规范：
vue
...
* defineProps 和 defineEmits 都是只能在  中使用的编译器宏。它们不需要导入，且会随着  的处理过程一同被编译掉。
...
### 响应式 Props 解构  
在 Vue 3.5 及以上版本中，从 defineProps 返回值解构出的变量是响应式的。当在同一个  块中的代码访问从 defineProps 解构出的变量时，Vue 的编译器会自动在前面添加 props.。
ts
...
* 不要为已经可以用  定义的选项使用单独的  部分，如 props 和 emits。
