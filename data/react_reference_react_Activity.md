...
```
当 Activity 边界被 隐藏 时，React 会使用 CSS 属性 display: "none" 从视觉上隐藏 其子组件。同时，React 还会销毁它们的 Effect，并清理所有活跃的订阅。
...
#### Caveats
- 如果 Activity 被渲染在 ViewTransition 内部，并且由于 startTransition 触发的更新而变为可见，它将触发 ViewTransition 的 enter 动画。如果它变为隐藏，则会触发其 exit 动画。
...
```
App.jsSidebar.jsApp.jsReloadClearForkimport { useState } from 'react';
...
然后看看新的行为表现：
```
App.jsSidebar.jsApp.jsReloadClearForkimport { Activity, useState } from 'react';
...
```
App.jsTabButton.jsHome.jsContact.jsContact.jsReloadClearForkexport default function Contact() {
  return (
    <div>
...
```
这是因为我们在 App 组件中完全卸载了 Contact。当Contact组标签页被卸载时，<textarea> 元素的内部 DOM 状态也就丢失了。
如果我们改用 Activity 边界来显示和隐藏当前活跃的标签页，我们就能保留每个标签页的 DOM 状态。试着再次输入文本并切换标签页，你会发现草稿消息不再被重置：
```
App.jsTabButton.jsHome.jsContact.jsApp.jsReloadClearForkimport { Activity, useState } from 'react';
...
```
当 Activity 边界在初次渲染时处于 隐藏 状态，其子组件虽然不会在页面上显示，但它们 仍会被渲染。不过，它们的渲染优先级会低于可见内容，且不会挂载它们的 Effect。
...
```
App.jsHome.jsPosts.jsApp.jsReloadClearForkimport { useState, Suspense } from 'react';
import TabButton from './TabButton.js';
import Home from './Home.js';
...
现在试着点击 Posts 标签页：
```
App.jsHome.jsPosts.jsApp.jsReloadClearForkimport { Activity, useState, Suspense } from 'react';
import TabButton from './TabButton.js';
import Home from './Home.js';
import Posts from './Posts.js';
...
- 使用 lazy 延迟加载（Lazy-loading）组件代码
...
```
App.jsHome.jsVideo.jsApp.jsReloadClearForkimport { useState } from 'react';
import TabButton from './TabButton.js';
import Home from './Home.js';
...
```
App.jsHome.jsVideo.jsApp.jsReloadClearForkimport { Activity, useState } from 'react';
import TabButton from './TabButton.js';
import Home from './Home.js';
import Video from './Video.js';
...
为了能及早发现其他没有正确清理逻辑的 Effect（这不仅对 Activity 边界至关重要，对 React 的许多其他行为也同样重要），我们建议使用 <StrictMode>.
### 隐藏组件中的 Effect 没有运行
...
为了及早发现有问题的 Effect，我们建议添加 <StrictMode>。它会主动执行 Activity 的卸载和挂载操作，从而捕获任何非预期的副作用。