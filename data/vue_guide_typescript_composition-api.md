...
## 为组件的 props 标注类型 
### 使用  
当使用  时，defineProps() 宏函数支持从它的参数中推导类型：
vue
...
这同样适用于 Props 从另一个文件中导入的情况，例如通过相对路径导入、路径别名 (如 @/types) 或外部依赖 (如 node_modules) 导入。该功能要求 TypeScript 作为 Vue 的一个 peer dependency。
vue
...
### 非  场景下 
如果没有使用 ，那么为了开启 props 的类型推导，必须使用 defineComponent()。传入 setup() 的 props 对象类型是从 props 选项中推导而来。
ts
...
## 为组件的 emits 标注类型 
在  中，emit 函数的类型标注也可以通过运行时声明或是类型声明进行：
vue
