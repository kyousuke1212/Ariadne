...
```
index.jsindex.htmlApp.jsindex.jsReloadClearForkimport './styles.css';
...
```
要对整个文档进行激活处理，将全局的 document 作为 hydrateRoot 的第一个参数传递：
```
...
```
index.jsindex.htmlApp.jsApp.jsReloadClearForkexport default function App() {
  return (
...
```
index.jsindex.htmlApp.jsApp.jsReloadClearForkimport { useState, useEffect } from "react";
...
### 更新激活根组件
在根组件激活完成之后，你可以调用 root.render 来更新根 React 组件。与  createRoot 不同的是，通常你不需要这样做，因为初始内容已经渲染为 HTML。
如果在激活之后某个时刻调用了 root.render，并且组件树结构与之前渲染的相匹配，那么 React 将 保留重置 state。请注意，你可以在输入框中输入文字，这意味着在此示例中每秒钟重复调用的 render 不会破坏已有的组件状态：
```
index.jsindex.htmlApp.jsindex.jsReloadClearForkimport { hydrateRoot } from 'react-dom/client';
import './styles.css';
import App from './App.js';
...
```
index.jsreportError.jsApp.jsindex.jsReloadClearForkimport { hydrateRoot } from "react-dom/client";
import App from "./App.js";
import {
  onCaughtErrorProd,
