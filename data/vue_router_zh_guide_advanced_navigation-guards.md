# 导航守卫 
在 Vue School 上观看免费视频课程✨自信地编写 Vue 应用的 Vibe 代码RuleKit
...
```
jsrouter.beforeEach(async (to, from) => {
  if (
    // 检查用户是否已登录
    !isAuthenticated &&
    // ❗️ 避免无限重定向
...
```
jsrouter.beforeResolve(async to => {
...
```
main.tstsconst app = createApp(App)
...
```
jsfunction removeQueryParams(to) {
...
```
jsbeforeRouteEnter (to, from, next) {
  next(vm => {
    // 通过 `vm` 访问组件实例
  })
}
